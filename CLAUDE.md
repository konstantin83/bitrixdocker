...
Доменно сгруппированная документация по инфраструктуре и эксплуатации. При разборе соответствующей темы — сначала смотреть сюда.

- `docs/security/` — защита и инциденты безопасности.
  - `zashchita-ot-parsinga.md` — объяснение для владельца (нетехническое): что за атаки парсингом, что предпринято.
  - `zashchita-ot-parsinga-tech.md` — техническая шпаргалка: конфиги fail2ban (`parser`, `parser-slow`), методика анализа логов nginx (профиль нагрузки, топ IP/подсетей, определение ASN, разбор `rt`/`urt`), журнал наблюдений.
  - **Когда открывать:** разбор атак/парсинга/DDoS, всплески 503/429, перегрузка сервера, настройка/калибровка блокировок, анализ статистики по нагрузке.

## Локальное тестирование доработок (обязательно)

Любую нашу доработку **всегда** проверять локально, очень внимательно, на поднятом Docker-окружении проекта. Порядок:

1. **Перед подъёмом** убедиться, что нет других запущенных Docker-контейнеров, которые займут наши порты (nginx `127.0.0.1:80/443`, MariaDB `127.0.0.1:3306`, Redis `6379`). Частые конфликтующие соседи: avito devenv (Lima/k3s, держит 80/443 — гасится `avito devenv stop`) и сторонние compose-проекты (например `bitrix24` со своим `redis-cache` на 6379). Запомнить, что именно останавливаем, чтобы потом вернуть.
2. Если конфликтующие контейнеры/среды запущены — **поставить их на паузу / остановить**, чтобы не мешали нашим.
3. Поднять окружение проекта: `make up` в корне. Дождаться, пока все контейнеры `krepcom-*` станут `Healthy`.
4. Прогнать проверку доработки, убедиться в результате очень тщательно.
5. **После того как всё проверено и результат полностью устраивает** — завершить контейнеры этого проекта (`make stop`/`make down` в корне) и **вернуть ранее остановленные** контейнеры/среды в исходное состояние (`avito devenv start`, `docker start <контейнер>` и т. п.).
